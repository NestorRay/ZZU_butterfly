    ...
    else    // 已经完成初始化，启动任务
    {
        ...
        if( now_time_ms - last_time_ms_control >= YDIFLY_CONTROL_CYCLE )
        {
            ...
            if( ydifly.remote.freq > 10 )    // 如果有给油门，则进入起飞程序
            {
                ...
                angle_r_max = YDIFLY_SERVO_ANGLE_R_INIT + ydifly.remote.yaw*YDIFLY_FACTOR_YAW - ydifly.remote.pitch*YDIFLY_FACTOR_PITCH + ydifly.remote.offset*YDIFLY_FACTOR_OFFSET + ydifly.remote.amp;
                angle_r_min = YDIFLY_SERVO_ANGLE_R_INIT - ydifly.remote.yaw*YDIFLY_FACTOR_YAW - ydifly.remote.pitch*YDIFLY_FACTOR_PITCH + ydifly.remote.offset*YDIFLY_FACTOR_OFFSET - ydifly.remote.amp;
